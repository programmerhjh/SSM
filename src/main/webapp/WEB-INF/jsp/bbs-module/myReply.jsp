...
<div class="page-container">
    <div class="container">
        <div class="row">
            ...
            <h3 style="text-align: center">我的回复列表</h3>

            <div class="span8 main-listing" id="listReply">

                <c:forEach items="${requestScope.page.list}" var="status" varStatus="s">

                    <article class="format-standard type-post hentry clearfix">

                        <p><h3>${status.replyReply}</h3> </p>

                        <h3><a class="readmore-link" href="postSpecific?postId=${status.post.postId}">To Post: ${status.post.postName} </a></h3>

                        <header class="clearfix">

                            <div class="post-meta clearfix">
                                <span class="date">回复时间 ：${status.replyCreatetime}</span>
        ...
        <div id="pagination">
            ...
            <select  class="btn" id="selectReplyPage" >
                ...
                <c:forEach items="${requestScope.page.navigatepageNums}" var="pg" varStatus="pgs">
                    <c:if test="${pgs.count != page.pageNum}">
                        <option value="${pgs.count}" id="chooseReplyPage${pgs.count}" >第 ${pgs.count} 页</option>
                    </c:if>
                </c:forEach>
            </select>
        </div>
...
<script type="text/javascript">
    $("option[id^='chooseReplyPage']").live('click',function () {
        ...
        $.ajax({
            ...
            success:function (page) {
                ...
                $("#listReply").empty()
                $("#selectReplyPage").empty()
                list.forEach(function (e) {
                    $("#listReply").append(

                        "<article class='format-standard type-post hentry clearfix'>"+

                        "<p><h3>"+e['replyReply']+"</h3></p>"+

                        " <a class='readmore-link' href='postSpecific?postId="+ e['post']['postId'] +"'>"+"To Post: "+ e['post']['postName'] +"</a>"+

                        "<header class='clearfix'>"+

                        "<div class='post-meta clearfix'>"+

                        "<span class='date'> 回复时间："+ e['replyCreatetime'] +"</span>"+
                ...
                list2.forEach(function (e) {
                    ...
                    if(e != page['pageNum']){
                        $("#selectReplyPage").append(
                            "<option value='"+ e +"' id='chooseReplyPage" + e + "' >"+"第" + e + " 页</option>"
                        )
                    }
